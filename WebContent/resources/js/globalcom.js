$(function() {
	...
	$(".password_generator").click(function() {
		$.getJSON("./user?action=randpass&callback=?", function(data) {
			if (data['password'] == null) {
				alert('Internal Server Error');
			} else {
				$('#fld_14').val(data['password']);
				$('#fld_15').val(data['password']);
			}
		});
	});
	...
	$("#add_item").click(
			function() {
				...
				if (!doneJSON) {
					$(this).hide();
					$.getJSON("./order?action=items&currencyId=" + currencyId
							+ "&callback=?", function(data) {
						if (data != null) {
							showLighbox();
							$.each(data,
									function(index, value) {
										var item = new Array(index,
												value['Description'],
												value['Price'],
												value['CurrencyId'],
												value['is_editable'],
							...
							doneJSON = true;
							initializeItems(items, existingItems);
	...
	$('.invoice_checkbox input').click(
			function() {
				$('#invoice_amount_field').val(
						$(this).parent().parent().children().children(
								'.selected_invoice_amount').text());
			});
	...
	$('.clickable_invoice_td').click(
			function() {
				 ...
				 * if(input.attr('checked')==true) { input.attr('checked',
				 * false);
				 * //removeInvoice($(this).children(".invoice_id").text(),
				 * invoicesCounter); //invoicesCounter--; } else{
			...
			.click(
					function() {
						...
						$('.payment_shared_fields .fieldwrapper')
								.each(
										function() {
											if ($(this).children('.fieldinput')
													.children('input').attr(
															'name') == "fld_payment_amount") {
												inamount = $(this).children(
														'.fieldinput')
														.children('input')
														.val();
											} else if ($(this).children(
													'.fieldinput').children(
													'input').attr('name') == "fld_payment_date") {
												indate = $(this).children(
														'.fieldinput')
														.children('input')
														.val();
											}
						...
						if ($(this).val() == '10') {
							if ($('.payment_cash_form').children(
									'.payment_shared_fields').hasClass('empty')) {
								var shared_fields = $('.payment_cheque_form')
										.children('.payment_shared_fields')
										.html();
								$('.payment_cheque_form').children(
										'.payment_shared_fields').empty();
								$('.payment_cash_form').children(
										'.payment_shared_fields').append(
										shared_fields);
								$('.payment_cash_form').children(
										'.payment_shared_fields').removeClass(
										'empty');
								$('.payment_cheque_form').children(
										'.payment_shared_fields').addClass(
										'empty');
								$('.payment_shared_fields .fieldwrapper')
										.each(
												function() {
							...
							if ($('.payment_cheque_form').children(
									'.payment_shared_fields').hasClass('empty')) {
								var shared_fields = $('.payment_cash_form')
										.children('.payment_shared_fields')
										.html();
								$('.payment_cash_form').children(
										'.payment_shared_fields').empty();
	...
	$('#payment_add_invoice').click(
			function() {
				showLighbox();
				var customer = $('#customer_query_string').text();
				var customerID = $('#customer_id_query_string').text();
				if (!paymentDoneJSON) {
					$.getJSON('./payment?action=list_unpaid_inv&' + customer
							+ '=' + customerID + '&callback=?', function(data) {
						if (data != null) {
	...
	$('.payment_available_invoices_tr').live('mouseover', function() {
		$(this).css({
			backgroundColor : '#E7EBF0'
		});
	});

	$('.payment_available_invoices_tr').live('mouseout', function() {
		$(this).css({
			backgroundColor : 'white'
		});
	});

	$('.payment_available_invoices_tr').live(
			'click',
			function() {
				;
				var payment = $('#payment_query_string').text();
				var invoice = $('#invoice_query_string').text();
				var paymentId = $('#payment_id_query_string').text();
				var invoiceId = $(this).children('#attach_invoice_id').text();
				var invoiceDate = $(this).children('#attach_invoice_date')
						.text();
				var invoiceAmount = $(this).children('#attach_invoice_amount')
						.text();
				var invoiceTotalAmount = $(this).children(
						'#attach_invoice_total_amount').text();
				showLighbox();
				$.getJSON('./payment?action=link&' + invoice + '=' + invoiceId
						+ '&' + payment + '=' + paymentId + '&callback=?',
						function(data) {
							if (data == 901) {
								alert($('#greater_invoice_error').text());
							} else if (data != null) {
								addInvoiceRow(invoiceId, invoiceDate,
										data['balance'], invoiceTotalAmount);
								showHideLink();
								$('#payemnt_amount_changeable').text(
										data['amount']);
								$('#payemnt_balance_changeable').text(
										data['balance']);
								$('#changeable_payment_balance').text(
										data['balance']);
								var balance = parseInt($(
										'#changeable_payment_balance').text(),
										10);
								if (data['can_pay_more'] == true) {
									$('#payment_add_invoice').show();
									$(this).remove();
								} else {
									$('#payment_add_invoice').hide();
								}
								if (balance == 0) {
									$('#payment_add_invoice').hide();
								} else {
								...
								$('.payment_available_invoices').remove();
							} else {
								alert('Internal Server Error');
							}
							$('#lightbox').css({
	...
	$('.remove_invoice').live(
			...
			function() {
				...
				var invoice = $('#invoice_query_string').text();
				var paymentId = $('#payment_id_query_string').text();
				var invoiceId = $(this).parent().parent().children(
						'.attached_invoice_id').text();
				showLighbox();
				;
				$.getJSON('./payment?action=unlink&' + invoice + '='
						+ invoiceId + '&' + payment + '=' + paymentId
						+ '&callback=?', function(data) {
					if (data != null) {
						$('#payemnt_amount_changeable').text(data['amount']);
						$('#payemnt_balance_changeable').text(data['balance']);
						if (data['can_pay_more'] == true) {
							$('#payment_add_invoice').show();
						} else {
							$('#payment_add_invoice').hide();
	...
	$('#filtration_box_input').live('keyup', function() {
		var myRegExp = new RegExp($(this).val().toUpperCase());
		var string1 = "";
		var matchPos1 = -1;
		$('.otheritemstable table .itemrow').each(function() {
			string1 = $(this).children('.search_phrase').text().toUpperCase();
